...
BST *check1(char UserName[], BST *h)
{
    if (h == NULL)
        return NULL;
    if (strcmp(UserName, h->student.UserName) == 0)
        return h;
    else if (strcmp(UserName, h->student.UserName) < 0)
        return check1(UserName, h->left);
    else
        return check1(UserName, h->right);
}
...
BST *ChangePass(BST *check)
{
    ...
    while (1)
    {
        ...
        printf("Xac nhan lai new Password: ");
        gets(newPass2);
